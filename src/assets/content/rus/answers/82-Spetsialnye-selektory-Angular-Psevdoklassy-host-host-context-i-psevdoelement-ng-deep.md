...
<h3>::ng-deep (deprecated) - использовать с осторожностью </h3>
<p>
...
<p>
  ...
  Если псевдокласс <code>::ng-deep</code> используется без селектора псевдокласса <code>:host</code>,
  стиль может просачиваться в другие компоненты.
</p>
<p class="info info--blue">
  По возможности используйте <code>::ng-deep</code> только в сочетании с <code>:host</code> для ограничения области действия стиля.
</p>
<p>
...
<p>
  Псевдокласс <code>::ng-deep</code> также имеет псевдонимы <code>>>></code> и <code>/deep/</code>.
</p>
<p>
  ...
  (для всех 3 псевдоклассов: <code>/deep/</code>, <code>>>></code> и <code>::ng-deep</code>).
</p>
<p class="info info--blue">
...
<p class="info info--orange">
  Angular Team пометила <code>::ng-deep</code> как устаревший еще в 2017 году, однако он до сих пор отлично работает в Angular 17, 18 и, скорее всего, никуда не исчезнет в ближайшее время. Потому что нативной, на 100% удобной альтернативы для всех случаев жизни пока не придумали.
  <br>
  Главная проблема <code>::ng-deep</code> в том, что он опирается на устаревшую спецификацию Shadow DOM, которую браузеры давно отклонили.
</p>
